...
void config_pins_for_ad(void)
{
    ...
    //TRISAbits.TRISA3 = 1; // Configure pin 8 for input MIDDLE SENSOR - JACOB
    //TRISBbits.TRISB4 = 1; // Configure pin 9 for input RIGHT SENSOR - JACOB
...
void config_ad(void)
{
    ...
    // AD1CSS registers
    AD1CSSL = 0b0011000000000011; // choose A2D channels you'd like to scan here.
    //this is doing pins AN 0,1,12,13 which is pins 7,8,9
...
double freq_left = 0;
//this value tells us which bin to put the ball, left = 1 is left, left = 0 is right
...
//        TOF_r = readRangeContinuousMillimeters(&(sensors[LEFT]));
       TOF_samp = readRangeContinuousMillimeters(&(sensors[FRONT]));
        
        //line_follower(leftval,midval,rightval);
        switch(state){
            case 1: //line following, looking for the bin
                line_follower(leftval, midval, rightval);
                if(TOF_samp <= 250 && ball == 1){
                    OC1RS = 12049;
                    OC2RS = 12049;
                    OC1R = 6000;
                    OC2R = 6000;
                ...
                break;
            case 2: // reading the ball value to see if black or white
                if(sampleval <= 2){
                    _OC1IE = 1;
                    white = 1;
                    steps = 0;       
                    state = 3; //3 is left, 4 is right, will change names later
                }
                else{
                    _OC1IE = 1;
                    white = 0;
            ...
            case 5:
                ...
                else if(steps >= 118 && ball == 0){
                    steps = 0;
                    state = 3;
                }
                break;
            case 6:
                //put code to turn on the correct solenoid pins depending on if left = 1 or not HERE
                // Start Jacob
                if (white == 1){
                    _LATB14 = 1;
                }
